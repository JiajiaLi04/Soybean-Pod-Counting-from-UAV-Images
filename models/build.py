...
from .backbones.swin_transformer_crowd import SwinTransformerCrowd
from .swin_crowd import SwinCrowd
...
def build_model(config, is_pretrain=False):
    ...
    if model_type == 'swin':
        ...
        model = SwinMLP(img_size=config.DATA.IMG_SIZE,
                        ...
                        use_checkpoint=config.TRAIN.USE_CHECKPOINT)
    elif model_type == 'swin_crowd':
        # backbone = SwinTransformerCrowd(img_size=config.DATA.IMG_SIZE,
        #                         patch_size=config.MODEL.SWIN.PATCH_SIZE,
        #                         in_chans=config.MODEL.SWIN.IN_CHANS,
        #                         num_classes=config.MODEL.NUM_CLASSES,
        #                         embed_dim=config.MODEL.SWIN.EMBED_DIM,
        ...
        backbone = SwinTransformerCrowd(pretrain_img_size=config.DATA.IMG_SIZE,
                                patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                in_chans=config.MODEL.SWIN.IN_CHANS,
                                embed_dim=config.MODEL.SWIN.EMBED_DIM,
                                depths=config.MODEL.SWIN.DEPTHS,
        ...
        model = SwinCrowd(backbone, 2, 2, embed_dim=config.MODEL.SWIN.EMBED_DIM)
